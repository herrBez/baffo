	...
	for {
		...
		case ast.Branch:
			...
			if baseConstraint != nil {
				script = script + fmt.Sprintf("field('%s.%s-base').set(%s);\n", TRANSPILER_PREFIX, branchName, *baseConstraint)
			} else {
				script = script + fmt.Sprintf("field('%s.%s-base').set(true);\n", TRANSPILER_PREFIX, branchName)
			}
			...
			if baseConstraint != nil {
				script = script + fmt.Sprintf("field('%s.%s-if').set(%s);\n", TRANSPILER_PREFIX, branchName, *baseConstraint)
			}
			...
			script = script + fmt.Sprintf("field('%s.%s-else').set(%s);", TRANSPILER_PREFIX, branchName, *transpileConstraint(currentConstraints))
			...
			if !t.fidelity {
				...
				cond = new(fmt.Sprintf("ctx.%s['%s-if']", TRANSPILER_PREFIX, branchName))
			}
			...
			for i := range block.ElseIfBlock {
				...
				if !t.fidelity {
					...
					cond = new(fmt.Sprintf("ctx.%s['%s-elseif-%d']", TRANSPILER_PREFIX, branchName, i))
				}
			...
			if !t.fidelity {
				...
				cond = new(fmt.Sprintf("ctx.%s['%s-else']", TRANSPILER_PREFIX, branchName))
			}
			// mergeWithIP(ip, tmp_ip, currentConstraints, t.threshold)
			t.mergeWithIPFidelity(ip, tmp_ip, cond)
